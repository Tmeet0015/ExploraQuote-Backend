...
const DestinationLocationRepository = AppDataSource.getRepository(DestinationLocation);
 ...
 //#region Destination
  ...
  export const createDestination = async (req: Request, res: Response) :Promise<any> => {
    try {
      ...
      if (existingDestination) {
        return res.status(400).json(
          CreateErrorResponse("Error", "Destination name already exists!", "Duplicate")
        );
      }
  ...
  export const getAllDestination = async (req: Request, res: Response) :Promise<any> => {
    try {
      ...
      const allowedSortFields = ["destination_id", "destination_name", "created_at"];
      const allowedStatuses = ["active", "inactive", "archived"];
      const allowedTypes = ["domestic", "international"];
  
      if (!allowedSortFields.includes(sortBy)) {
        return res.status(400).json(
          CreateErrorResponse("Error", "Invalid sortBy field!", "Invalid")
        );
      }
      ...
      if (type && !allowedTypes.includes(type)) {
        return res.status(400).json(
          CreateErrorResponse("Error", "Invalid type filter!", "Invalid")
        );
      }
      ...
      if (total === 0) {
        return res.status(404).json(
          CreateErrorResponse("Error", "No data found!", "NotFound")
        );
      }
  ...
  export const getOneDestination = async (req: Request, res: Response) :Promise<any> => {
    try {
      const { id } = req.params;

      if (!id || Number.isNaN(Number(id))) {
        return res.status(400).json(
          CreateErrorResponse("Error", "Invalid ID!", "Invalid")
        );
      }
  ...
  export const deleteDestination = async (req: Request, res: Response) :Promise<any> => {
    try {
      ...
      const result = await DestinationRepository.delete({ destination_id: parseInt(id) });
  ...
  // Read All Location
  export const getAllLocation = async (req: Request, res: Response) : Promise<any> => {
    try {
  ...
  // Read One Location
  export const getOneLocation = async (req: Request, res: Response) : Promise<any> => {
    try {
      const { id } = req.params;
  ...
  export const updateLocation = async (req: Request, res: Response) : Promise<any> => {
    try {
      ...
      if (updateResult.affected === 0) {
        return res.status(404).json(
          CreateErrorResponse("Error", "Location not found!", "NotFound")
        );
      }
  ...
  export const deleteLocation = async (req: Request, res: Response) : Promise<any> => {
    try {
      ...
      const result = await LocationRepository.delete({ location_id: parseInt(id) });

      if (result.affected === 0) {
        return res.status(404).json(
          CreateErrorResponse("Error", "Location not found!", "Invalid")
        );
      }
...
export const getAllDestinationLocations = async (req: Request, res: Response): Promise<any> => {
  try {
...
export const getDestinationLocationById = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
    ...
    if (!destinationLocation) {
      return res.status(404).send(CreateErrorResponse("Error", "Destination-Location not found!", "Invalid"));
    }

    return res.status(200).send(CreateSuccessResponse("Destination-Location found successfully!", destinationLocation));
  } catch (error) {
    const errorlog = {
...
export const addDestinationLocation = async (req: Request, res: Response): Promise<any> => {
  try {
    const { destination_id, location_id } = req.body;

    // Validation: Check if the destination-location combination already exists
    const existingDestinationLocation = await DestinationLocationRepository.findOne({
      where: { destination: { destination_id }, location: { location_id } },
    });

    if (existingDestinationLocation) {
      return res.status(400).send(CreateErrorResponse("Error", "This destination-location combination already exists!", "Invalid"));
    }

    // Save the new destination location
    await DestinationLocationRepository.save({destination : destination_id, location : location_id, });

    return res.status(201).send(CreateSuccessResponse("Destination-Location created successfully!"));
  } catch (error) {
    const errorlog = {
...
export const updateDestinationLocation = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
    ...
    // Validation: Check if the destination-location combination already exists
    const existingDestinationLocation = await DestinationLocationRepository.findOne({
      where: { destination: { destination_id }, location: { location_id }, destinationLocation_id: Not(parseInt(id)) },
    });
    ...
    destinationLocation.destination = destination_id || destinationLocation.destination;
    ...
    return res.status(200).send(CreateSuccessResponse("Destination Location updated successfully!", destinationLocation));
  } catch (error) {
    const errorlog = {
...
export const deleteDestinationLocation = async (req: Request, res: Response): Promise<any> => {
  try {
    const { id } = req.params;
    ...
    return res.status(200).send(CreateSuccessResponse("Destination Location deleted successfully!"));
  } catch (error) {
    const errorlog = {
