...
export const createFlightDetails = async (req: Request, res: Response) => {
  try {
    ...
    if (existingFlight) {
      return res
        .status(400)
        .json({ error: "Duplicate entry. Flight details already exist." });
    }
...
export const deleteFlightDetails = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const flightDetails = await flightDetailsRepository.findOneBy({
    ...
    if (!flightDetails) {
      return res
        .status(404)
        .json(
          CreateErrorResponse("Error", "Not Found", "Flight details not found.")
        );
    }
    ...
    return res
      .status(200)
      .json({ message: "Flight details deleted successfully." });
  } catch (error) {
    const errorlog = {
