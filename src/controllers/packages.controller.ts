...
const packageDestLocationRepository =
  AppDataSource.getRepository(PackageDestLocation);
...
export const updatePackage = async (req: Request, res: Response) => {
  try {
    ...
    if (
      Array.isArray(DestinationLocationIds) &&
      DestinationLocationIds.length > 0
    ) {
      await packageDestLocationRepository.delete({
...
export const deletePackage = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const deleteResult = await packageRepository.update(id, {
      is_active: false,
    });
